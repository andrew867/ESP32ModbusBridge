/**
 * @file crc_utils.c
 * @brief CRC calculation implementation
 */

#include "crc_utils.h"

// Modbus CRC-16 lookup tables
static const uint8_t crc_table_high[256] = {
    
};

static const uint8_t crc_table_low[256] = {
    
};

/**
 * @brief Calculate Modbus CRC-16
 * 
 * Original: sub_420157EE
 * The original function used a lookup table approach similar to this.
 */
uint16_t modbus_crc16(const uint8_t *data, uint16_t length)
{
    uint8_t crc_high = 0xFF;
    uint8_t crc_low = 0xFF;
    uint8_t index;
    
    for (uint16_t i = 0; i < length; i++) {
        index = crc_low ^ data[i];
        crc_low = crc_high ^ crc_table_high[index];
        crc_high = crc_table_low[index];
    }
    
    return (crc_high << 8) | crc_low;
}

/**
 * @brief Verify Modbus CRC
 */
int modbus_verify_crc(const uint8_t *frame, uint16_t length)
{
    if (length < 2) {
        return -1; // Frame too short
    }
    
    // Calculate CRC for frame without CRC bytes
    uint16_t calculated_crc = modbus_crc16(frame, length - 2);
    
    // Get CRC from frame (little-endian)
    uint16_t frame_crc = frame[length - 2] | (frame[length - 1] << 8);
    
    return (calculated_crc == frame_crc) ? 0 : -1;
}

